...
The prepared patch was checked against both current `mizchi/x` `main` and the registry contents for `0.5.2`. In an upstream checkout, native Windows and WASM checks pass for the affected `fs`, `socket`, `pipe`, and `stdio` packages with warnings denied; the focused native socket, pipe, and stdio tests pass (16/16). Applied temporarily to `pumd`'s resolved dependency cache, `moon check --target native` passes and the full native suite passes (343/343). The cache was restored after validation.

The durable resolution remains a new `mizchi/x` release containing this patch, pinned together with `moonbitlang/async@0.20.3`. Until that release exists, an explicitly authorized fork or vendored dependency containing exactly these changes is the reproducible alternative.
